...
public class BaseProperties {
    ...
    public BaseProperties (BaseProperties props) {
        this.bootstrapServers = props.getBootstrapServers();
        this.consumerGroup = props.getConsumerGroup();
        this.sessionTimeout = props.getSessionTimeout();
        this.heartbeatInterval = props.getHeartbeatInterval();
        this.autoOffsetReset = props.getAutoOffsetReset();
        this.maxWait = (props.getMaxWait() < 0) ? 0 : props.getMaxWait();
        this.maxPartitionFetchBytes = (props.getMaxPartitionFetchBytes() < 1) ? 1 : props.getMaxPartitionFetchBytes();
        this.minBytes = (props.getMinBytes() < 0) ? 0 : (props.getMinBytes() > props.getMaxPartitionFetchBytes())
                ? props.getMaxPartitionFetchBytes() : props.getMinBytes();
        this.sendBufferBytes = (props.getSendBufferBytes() < 1) ? 1 : props.getSendBufferBytes();
        this.receiveBufferBytes = (props.getReceiveBufferBytes() < 1) ? 1 : props.getReceiveBufferBytes();
        this.clientId = props.getClientId();
        this.reconnectBackoffMs = (props.getReconnectBackoffMs() < 1) ? 1 : props.getReconnectBackoffMs();
        this.retryBackoffMs = (props.getRetryBackoffMs() < 1) ? 1 : props.getRetryBackoffMs();
        this.metadataMaxAgeMs = (props.getMetadataMaxAgeMs() < 1000L) ? 1000L : props.getMetadataMaxAgeMs();
        this.connectionsMaxIdleMs = (props.getConnectionsMaxIdleMs() < 1000L) ? 1000L : props.getConnectionsMaxIdleMs();
        this.requestTimeoutMs = (props.getRequestTimeoutMs() < 1) ? 1 : props.getRequestTimeoutMs();
        this.maxPollRecords = (props.getMaxPollRecords() < 1) ? 1 : props.getMaxPollRecords();

        this.maxMessagesBeforeCommit = (props.getMaxMessagesBeforeCommit() < 1) ? 1 : props.getMaxMessagesBeforeCommit();
        this.maxTimeBeforeCommit = (props.getMaxTimeBeforeCommit() < 0) ? 0 : props.getMaxTimeBeforeCommit();
        this.hwmRefreshIntervalMs = (props.getHwmRefreshIntervalMs() < 1000) ? 1000 : props.getHwmRefreshIntervalMs();
    ...
    private BaseProperties(String bootstrapServers, String consumerGroup, int sessionTimeout, int heartbeatInterval,
    ...
    public static class Builder {
        ...
        protected BaseProperties build(Configuration config) {
            return new BaseProperties(
                    config.getBootstrapServers(), config.getConsumerGroup(), config.getSessionTimeout(), config.getHeartBeatInterval(),
                    config.getMinBytes(), config.getAutoOffsetReset(), config.getMaxWait(), config.getMaxPartitionFetchBytes(),
                    config.getSendBufferBytes(), config.getReceiveBufferBytes(), config.getClientId(), config.getReconnectBackoffMs(),
                    config.getRetryBackoffMs(), config.getMetadataMaxAgeMs(), config.getConnectionMaxIdleMs(), config.getRequestTimeoutMs(),
                    config.getMaxMessagesBeforeCommit(), config.getMaxTimeBeforeCommit(), config.getHwmRefreshIntervalMs(), config.getMaxPollRecords());
        }
    }
